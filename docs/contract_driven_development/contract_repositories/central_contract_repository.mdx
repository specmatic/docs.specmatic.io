...
Run `specmatic spec-metadata-report --ci` with build and repository metadata to generate and publish this report. See [Reports Sent to Insights](/enterprise_onboarding/insights/reports_sent_to_insights#central-contract-repository-report-payload) for the JSON payload.
...
jobs:
  publish-central-contract-repo-report:
    ...
    steps:
      ...
      - name: Generate and publish Central Contract Repo Report
        run: |
