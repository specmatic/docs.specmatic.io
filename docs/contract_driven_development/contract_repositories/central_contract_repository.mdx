...
In a central contract repository setup, all contracts/specifications are stored in one shared repository. Providers and consumers both point to that same repository.

In a central contract repository setup, all stakeholders refer to the same shared source of truth for contracts/specifications.
...
- `central-contract-repository` stores the `payment-service` contract at `contracts/services/payment-service/http/openapi.yaml`
- `payment-service` provides that contract from the central contract repository
- `order-service` consumes that same contract from the central contract repository
...
For this example, the relevant contract is:
...
`order-service` is the consumer in this example.
