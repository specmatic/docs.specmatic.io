...
1. a Spec Metadata Report
2. a service build report

### Spec Metadata Report

#### Generate the Spec Metadata Report
...
This generates the Spec Metadata Report at:

- `build/reports/specmatic/spec_metadata_report.json`

Run `specmatic spec-metadata-report --ci` with build and repository metadata to generate and publish this report. See [Reports Sent to Insights](/enterprise_onboarding/insights/reports_sent_to_insights#spec-metadata-report-payload) for the JSON payload.
...
1. Generate the Spec Metadata Report
2. Publish the Spec Metadata Report to Insights

For example:

```yaml
name: Spec Metadata Reports
...
jobs:
  publish-spec-metadata-report:
    ...
    steps:
      ...
      - name: Generate and publish Spec Metadata Report
        run: |
...
jobs:
  publish-service-build-report:
    ...
    steps:
      ...
      - name: Run Contract Test Suite and publish to Insights
        run: |
          docker run --rm -it \
            -v "$(pwd):/usr/src/app" \
            -v ~/.specmatic:/root/.specmatic \
...
### How are Spec Metadata Reports processed in Insights?

Every Spec Metadata Report pushed to Insights results in a `Spec Repository` block being rendered on the dashboard.
...
The `Spec Repository` block lists all the specs that were present in the Spec Metadata Report in a directory structure.
...
1. the Spec Metadata Report was generated from the central contract repository and posted to Insights
...
2. the Spec Metadata Report was generated and posted to Insights
