...
Since `web-bff` has the `schema.graphql` spec in its own `specs/` folder, its Spec Metadata Report creates a separate `Spec Repository` block for that GraphQL spec.
...
1. the `spec-metadata-report` was generated from the provider repository and posted to Insights
...
2. the provider's `spec-metadata-report` was generated and posted to Insights
