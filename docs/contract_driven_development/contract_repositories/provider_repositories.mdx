...
### How are Central Repo Reports processed in Insights?

Every central repo report pushed to Insights results in a `Spec Repository` block being rendered on the dashboard.

The `Spec Repository` block lists all the specs that were present in the central repo report in a directory structure.
...
If found, Insights will display the service repo name as the left-hand-side block and link it to the provider service in the center block.

If the center service/provider service also consumes other specifications, Insights will display those provider services on the right-hand side and link them to the center block as dependencies.
...
Since `web-bff` has the `schema.graphql` spec in its own `specs/` folder, its central repo report creates a separate `Spec Repository` block for that GraphQL spec.
