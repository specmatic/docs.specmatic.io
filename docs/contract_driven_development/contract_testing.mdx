...
Many frameworks can expose an OpenAPI document generated from the application's routes and models. For example,
Springdoc commonly exposes one at `/v3/api-docs`, while Swashbuckle can expose one at
`/swagger/v1/swagger.yaml` or `/swagger/v1/swagger.json`.

Set `swaggerUrl` to the **complete URL of that OpenAPI document**. Specmatic sends a `GET` request to this URL, parses
the returned OpenAPI document, and compares its operations with the contract test coverage. The endpoint must be
reachable from the environment in which Specmatic runs and return HTTP `200`.

Set `swaggerUrl` under `systemUnderTest.service.runOptions.openapi`:

<V3SpecmaticJsonYamlTabs v3Object={yaml.load(require('./includes/contract-testing/swagger-url/specmatic.yaml?raw'))} />

If the application exposes the generated document for a specific API specification at a different URL or port, set a
`swaggerUrl` override under `systemUnderTest.service.runOptions.openapi.specs[].spec`. The `id` must match the
corresponding specification in `systemUnderTest.service.definitions`. This value overrides the top-level `swaggerUrl`
