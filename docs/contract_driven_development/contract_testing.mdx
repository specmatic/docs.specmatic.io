...
<YamlDisplay object={yaml.load(require('./includes/contract-testing/product-api.yaml?raw'))} title='products.yaml'/>

### Setting Up Test Hooks

**Step 1**: Create a specmatic configuration file

<V3SpecmaticJsonYamlTabs v3Object={yaml.load(require('./includes/contract-testing/test-hooks/specmatic.yaml?raw'))} />
...
<Tabs groupId="hook_script">
  ...
  <TabItem value="python" label="Python">
```python title="modify_headers.py"
import os
import sys
import yaml
...
* Follow instructions in the project's [README.md](https://github.com/specmatic/specmatic-hooks-java-sample/blob/main/README.md)
...
* Update the `test_load_contract` hook command in your Specmatic configuration to point to this JAR file `java -jar specmatic-hooks-sample.jar`
...
> Specmatic will extract request and response from the `MonitorResponse` to reconstruct the original request and response for validation and further processing.- **Explicit Case**: The extracted request and response must align with any non-202 2xx scenarios defined in the specification- **Implicit Case**: The extracted request and response must conform to the scenario that was being tested, such as `201 Created`
&#123;: .note&#125;
