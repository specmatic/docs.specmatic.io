...
Add a class that implements `io.specmatic.test.SpecmaticContractTest` (open source) or `io.specmatic.enterprise.SpecmaticContractTest` (Specmatic Enterprise). See how this is done - [Interface based contract test example](https://github.com/specmatic/specmatic-order-api-java/blob/main/src/test/java/com/store/ContractTest.java)

Add the Specmatic configuration file at the project root, as described in the previous section.

Since this is a JUnit 5 test, you can run it the same way you run any other JUnit test. When you run it in your IDE, results will appear in the IDE test runner. When you run `mvn test` or `./gradlew test`, Maven or Gradle will write the contract test results to the standard JUnit XML reports alongside your other tests.
...
<Tabs groupId="programmatically">
  ...
  <TabItem value="java" label="Java/Kotlin">
    ...
    If you're building your application in a JVM-based language, you can run Specmatic Contract Tests programmatically, either by implementing `io.specmatic.test.SpecmaticContractTest` (open source) or `io.specmatic.enterprise.SpecmaticContractTest` (Specmatic Enterprise) Here's how:
...
4. **Configure and Run Contract Tests**: Use the Specmatic class to configure and run your contract tests. Here's an example of how to do this for a Flask application:
   ...
   In this example, replace `your_project` with your project's name and `app` with your Flask application object. If app_host and app_port are not specified, the app will be started on a random available port on 127.0.0.1.
...
<span id="summary" />
After the contract tests are completed, Specmatic generates HTML and CTRF reports. These reports provide insights into which operations were executed, which operations aligned with the specifications, and where discrepancies or missing specification entries were identified. **HTML** reports are intended for human-readable analysis, while **CTRF** reports are tailored for CI/CD integrations and downstream tooling.

To publish this CTRF report in CI, run `specmatic test --ci` with `--build-id`, `--repo-id`, `--repo-name`, `--repo-url`, and `--branch-name`. See [Reports Sent to Insights](/enterprise_onboarding/insights/reports_sent_to_insights#contract-test-report-payload).
...
## Default Responses in OpenAPI

An OpenAPI operation can use `default` to describe a response when the API can return statuses that are not listed individually. In contract testing, each test runs against a response definition selected from the OpenAPI specification. The `default` response can accept an undeclared status only when that response definition is being evaluated, for example through a default-response example or as a fallback during negative schema resiliency testing.

### Defining a default response

Declare `default` alongside the operation's numeric responses:
              ...
              schema:
                ...
                properties:
                  message:
                    type: string
        default:
          description: An error not described by another response
          content:
            application/json:
              schema:
                type: object
...
:::note
`default` is a fallback, not a catch-all. When the `default` response definition is being evaluated, it is eligible only for statuses that are not explicitly declared for that operation. If a test is selected for a different response definition, an undeclared status causes the test to fail; it does not switch to `default`.
:::

For the specification above, here is how response statuses in contract test responses are evaluated against the `default` response:

| Status returned by the application | Eligible for default response match? |
|---|---|
| `200` | Not eligible as spec explicitly declares 200 |
| `404` | Not eligible; explicitly declared |
| `400`, `401`, or `500` | Eligible; not explicitly declared |

This allows the specification to define a common schema for otherwise-undescribed error responses without listing every possible error status.

### Writing an example for a default response

An externalized example for a default response must use the concrete HTTP status that the application is expected to return:

```json title="items_unavailable.json"
{
  "http-request": {
    "method": "GET",
    "path": "/items",
  ...
  "http-response": {
    "status": 400,
    "body": {
      "message": "Items are temporarily unavailable"
    }
  }
}
```

Specmatic associates this example with the operation's `default` response when its request metadata matches the specification and its response status is eligible for the `default` response. Here, `400` is not explicitly declared, so the example is associated with `default`. Its response body must conform to the schema defined under `default`; in this case, `message` is required and must be a string.

:::warning
Use a real HTTP status in the example. `default` is an OpenAPI response key, not a value that can be used as the status.
:::

:::note
Inline examples under `responses.default` do not create tests for the default response. To exercise a default response, use an externalized example with the concrete HTTP status the application should return, as shown above.
:::

### How Specmatic matches the response

The response definition is selected before the application response is validated. For example:

| Response status being tested for | Application response | Result |
|---|---|---|
| `200` declared expicitly in spec | `200` | Validated as per the `200` response in spec |
| `200` declared expicitly in spec | `400` | Fail; the response is not validated against `default` |
| `default` response in spec and `400` status in example | `400` | Validate against the `400` status response in example |
| `default` response in spec and `400` status in example | `401` | Fail; the example expects `400` |

:::info
A test is tied to its selected response definition. If a test targets the explicit `200` response, an actual `400` response fails; it does not switch to `default`. Similarly, an example with status `404` is associated with the explicit `404` response, not `default`. If `400` is later declared explicitly, the example above is associated with that new `400` response.
:::

### Test generation

A `default` response does not generate tests, with or without generative testing, because it does not define a concrete status. To exercise a default response, provide an externalized example with the status the application should return. However, during negative schema resiliency testing, `default` will still be used as a fallback to validate an undeclared response status.
...
:::note
Values are available from the point at which they are captured. Fixtures in the same section are executed in order. This means a later `before` fixture can use values captured by an earlier `before` fixture. Similarly, an `after` fixture can use values captured by `before` fixtures, the main contract test response, and earlier `after` fixtures.
:::
...
Each `before` and `after` fixture can optionally specify when it should be executed by using `executeFor` at the fixture root. By default, fixtures are executed only for positive contract test scenarios. This means that omitting `executeFor` is the same as specifying:
```json
{
  "executeFor": {
    "scenarios": "positive"
...
For example, this fixture runs only for positive scenarios because `executeFor` is not specified. It creates a department and captures `DEPARTMENT_ID`, which can then be used by the main contract test.
...
Use `all` when a fixture should run for both positive and negative scenarios. For example, this fixture resets department test data after every scenario:
...
Substitution is lenient by default. This means that if Specmatic cannot resolve a substitution expression, it will not fail the test. Instead, it will log a warning and generate a value for the field based on the schema. However, this does not apply to fixtures, as there is no underlying schema to generate from.
...
After executing the tests, Specmatic generates a tabular API Coverage report showing which API operations were covered by the tests. Specmatic can also retrieve the operations exposed by the application through an actuator mappings endpoint or a generated OpenAPI/swagger document URL. This enables Specmatic to highlight operations that were not tested, operations defined in the specification but not implemented by the application, and operations exposed by the application but missing from the specification.

There are two methods for retrieving the operations exposed by the application:
...
<span id="use-swagger-ui"></span>

#### Use Swagger URL Endpoint

Many frameworks can expose an OpenAPI document generated from the application's routes and models. The following table lists the default or commonly configured document paths for popular framework integrations:

| Language/ecosystem | Framework or integration | OpenAPI document path | Notes |
| --- | --- | --- | --- |
| Java/Kotlin | [Spring Boot with springdoc-openapi](https://springdoc.org/v2/getting-started.html) | `/v3/api-docs` (JSON) or `/v3/api-docs.yaml` (YAML) | Configurable using `springdoc.api-docs.path`. |
| Java/Kotlin | [Quarkus with SmallRye OpenAPI](https://quarkus.io/guides/openapi-swaggerui) | `/q/openapi` | Add `?format=json` to request JSON. The path is configurable using `quarkus.smallrye-openapi.path`. |
| .NET | [ASP.NET Core built-in OpenAPI](https://learn.microsoft.com/aspnet/core/fundamentals/openapi/aspnetcore-openapi) | `/openapi/v1.json` | Available when the application calls `MapOpenApi()`. The document name and route can be customized. |
| .NET | [ASP.NET Core with Swashbuckle](https://learn.microsoft.com/aspnet/core/tutorials/getting-started-with-swashbuckle) | `/swagger/v1/swagger.json` | The document name and route can be customized. |
| Python | [FastAPI](https://fastapi.tiangolo.com/tutorial/metadata/#openapi-url) | `/openapi.json` | Configurable using the `openapi_url` parameter. |
| Python | [Django REST Framework with drf-spectacular](https://drf-spectacular.readthedocs.io/en/stable/readme.html#take-it-for-a-spin) | Commonly `/api/schema/` | There is no fixed default; the path is selected when registering `SpectacularAPIView`. |
| Node.js/TypeScript | [NestJS with `@nestjs/swagger`](https://docs.nestjs.com/openapi/introduction) | `<swagger-ui-path>-json` | If Swagger UI is mounted at `/api`, the JSON document is exposed at `/api-json`. The path can be customized. |

These paths can be changed by application configuration, routing prefixes, gateways, or reverse proxies.

Set `swaggerUrl` to the **complete URL of that OpenAPI document**. Specmatic sends a `GET` request to this URL, parses the returned OpenAPI document, and compares its operations with the contract test coverage. The endpoint must be reachable from the environment in which Specmatic runs, return the raw OpenAPI JSON or YAML document rather than the Swagger UI HTML page, and respond with HTTP `200`.

In your [Specmatic configuration](/references/configuration), set `swaggerUrl` under `systemUnderTest.service.runOptions.openapi`:

<V3SpecmaticJsonYamlTabs v3Object={yaml.load(require('./includes/contract-testing/swagger-url/specmatic.yaml?raw'))} />

If the application exposes the generated document for a specific API specification at a different URL or port, set a `swaggerUrl` override under `systemUnderTest.service.runOptions.openapi.specs[].spec`. The `id` must match the corresponding specification in `systemUnderTest.service.definitions`. This value overrides the top-level `swaggerUrl` for that specification.

If both are present, `actuatorUrl` is favored over `swaggerUrl`. However if a `swaggerUrl` override is present, the override is preferred over other options.

If neither is configured, Specmatic attempts to retrieve `<system-under-test-base-url>/swagger/v1/swagger.yaml`.

The test results, including coverage, can be accessed in the HTML and CTRF (Commercial Only) reports generated at `./build/reports/specmatic/test/html/index.html` and `./build/reports/specmatic/test/ctrf/ctrf-report.json`, respectively.
...
If we send a request containing the body `{"color": null, "category": "medicals"}`, the application should respond with a HTTP `400 Bad Request` status, because according to the specification, the value of `color` cannot be null.

<V3SpecmaticJsonYamlTabs
  v3Object={yaml.load(require('./includes/contract-testing/generative-tests/specmatic.yaml?raw'))}/>


Specmatic can generate this test (`color` cannot be `null`) and many more to ensure the required validations are in place, and it will do the same for `category`. With the Specmatic configuration shown above, generative tests are enabled by default. To disable them, set the environment variable `DISABLE_GENERATIVE_TESTS=true`.
...
<span id="accepted-response-handling" />
#### Accepted Response Handling <CommercialBadge/>
...
<span id="too-many-requests-handling" />
#### Too Many Requests Handling <CommercialBadge/>
