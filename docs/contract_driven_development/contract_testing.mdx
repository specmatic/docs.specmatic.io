...
For the specification above, the statuses eligible while evaluating the `default` response are:

| Status returned by the application | Default response |
|---|---|
| `200` | Not eligible; explicitly declared |
...
| Selected response definition | Application response | Result |
|---|---|---|
| Explicit `200` | `200` | Validate against the explicit `200` response |
| Explicit `200` | `400` | Fail; the test does not switch to `default` |
| `default` example with status `400` | `400` | Validate against the `default` response |
| `default` example with status `400` | `401` | Fail; the example expects `400` |
