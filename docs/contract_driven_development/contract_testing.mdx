...
A `default` response does not generate tests, with or without generative testing, because it does not define a concrete status.
To exercise a default response, provide an externalized example with the status the application should return. However, during negative schema resiliency testing, `default` will still be used as a fallback to validate an undeclared response status.
...
Many frameworks can expose an OpenAPI document generated from the application's routes and models. The following
table lists the default or commonly configured document paths for popular framework integrations:
