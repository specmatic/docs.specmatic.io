...
![Diagram showing Specmatic reverse proxy flow](/img/specmatic-reverse-proxy.svg)
...
Need help troubleshooting or have questions? Reach out to us at [Specmatic support](https://specmatic.io/contact-us).
...
The file must contain a single JSON object using the [Specmatic stub file format](/features/external_examples#example-format).
...
Now run the following command on it:

```bash
> specmatic import customer_stub.json
...
```bash
> specmatic import employee_postman_collection.json
```
