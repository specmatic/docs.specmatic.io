...
2. **[Central Contract Repo](/contract_driven_development/central_contract_repository)**: Treat the API specs as code and store it in a central Git repository. This central Git repo should be the single source of truth referenced by both providers and consumers.

   Teams that want providers to own their contracts in their own repositories can instead use **[Federated Repositories](/contract_driven_development/federated_repositories)**.

3. **[Backward Compatibility Checks](/contract_driven_development/backward_compatibility)** and **Linting**: Perform automated backward compatibility checks to ensure we are not accidentally breaking compatibility. Also performing #linting to ensure the #APIDesign meets the agreed-upon specification standards.
