...
![Anatomy of a Component / API Test](https://specmatic.io/wp-content/uploads/2022/09/Contact-as-mock.png)
...
Here is an API specification for a products API (backend):
  ...
  <YamlDisplay object={yaml.load(require('./includes/service-virtualization/product-api/product.yaml?raw'))} />
...
- Create a specmatic configuration file:

  <V3SpecmaticJsonYamlTabs v3Object={yaml.load(require('./includes/service-virtualization/product-api/specmatic.yaml?raw'))}/>

- Create the hook script (`modify_stub_header.py`):

  <CodeBlock title='modify_stub_header.py' language='python'>
  {require('./includes/service-virtualization/product-api/modify_stub_header.py?raw')}
  </CodeBlock>

### How It Works

When Specmatic runs in _mock_ mode:
1. Specmatic invokes the hook script specified in the `specmatic.yaml` configuration for the `stub_load_contract`
   event and passes the specification file path via the `CONTRACT_FILE` environment variable.
2. The hook script must then read the specification file as specified by the `CONTRACT_FILE` environment variable,
   modify it, and output the updated specification to standard output
3. Specmatic loads the modified specification to run the mock server

The hook script modifies the OpenAPI specification by:
- Reading the specification file
- Replacing the `X-internal-id` header parameter with `X-auth-token`
- Outputting the transformed specification for the mock server to use

This allows the mock to accept requests with the `X-auth-token` header that the frontend uses, while maintaining
the original specification for the backend API.

:::note
If you're looking to modify headers during test using Specmatic's `test` command, then please refer to test-specific header modifications documentation: [Using Hooks during Tests](/contract_driven_development/contract_testing#hooks)
:::
