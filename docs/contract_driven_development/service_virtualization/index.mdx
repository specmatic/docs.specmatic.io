...
## Retrieving Messages Served By The Mock Server <CommercialBadge />
...
The response contains one object for each request/response pair served by the mock server. For example:
...
[
  {
    "http-request": {
      ...
      "time-stamp": "2025-01-15T10:30:00.123Z"
    },
    "http-response": {
      "status": 200,
      "headers": {
      ...
      "time-stamp": "2025-01-15T10:30:00.456Z"
    }
  }
]
```
...
:::note
The `time-stamp` in `http-request` records when the request was received by the mock server. The `time-stamp` in `http-response` records when the response was served by the mock server. Both values use the RFC 3339 `date-time` format.
:::
