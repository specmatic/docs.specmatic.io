...
| field name                    | data type | description                                                                 |
|------------------------------|-----------|-----------------------------------------------------------------------------|
| `details.resourceType`       | `string`  | SCIM resource category for this event; user lifecycle events emit `USER`.   |
| `details.operation`          | `string`  | Provisioning operation recorded for the mutation; this event always emits `CREATE`. |
| `details.userName`           | `string`  | Canonical SCIM `userName` persisted for the created user link.              |
| `details.projectedUserEmail` | `string`  | Email on the projected local user record when linkage/projection is present. |
| `details.externalId`         | `string`  | Optional IdP-provided `externalId` stored with the SCIM user link.          |
...
| field name               | data type | description                              |
|-------------------------|-----------|------------------------------------------|
| `details.resourceType`  | `string`  | SCIM resource category for this event; group lifecycle events emit `GROUP`. |
| `details.operation`     | `string`  | Provisioning operation recorded for the mutation; this event always emits `CREATE`. |
| `details.displayName`   | `string`  | Group display name value persisted on the SCIM group resource. |
| `details.externalId`    | `string`  | Optional IdP-provided `externalId` associated with the SCIM group. |
