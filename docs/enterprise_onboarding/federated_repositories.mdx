...
### The provider report is missing in Insights

Check that:

- `central-contract-repo-report` was run from `specs/`
- the central report was sent before the service report
- the Docker run mounted the repository root

### The consumer build is visible but the provider contract is not

Check that:

- the consumer points to the provider repo and the correct spec path
- the federated central contract report for the provider was already processed
