...
:::note
Legacy build reporter support has been removed. If your pipeline still uses `specmatic-insights-build-reporter-github-action` or `specmatic-insights-github-build-reporter`, migrate to direct reporting from `specmatic test`, `specmatic mock`, or `specmatic run-suite` using the [Migration Guide](/references/migration_guide#2-migrate-from-legacy-insights-reporter-to-direct-command-reporting).
:::
