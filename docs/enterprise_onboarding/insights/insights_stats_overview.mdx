...
- Spec Metadata provides the contract baseline
...
For contract metrics, each weekly point represents the latest available Spec Metadata snapshot for that week. The snapshot is evaluated against the contract baseline and the service build data available at that point. The large value on each card is the value from the latest week in the selected range.
