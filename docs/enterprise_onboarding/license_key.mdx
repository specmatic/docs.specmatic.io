...
If you need the access model behind user licenses, service-account licenses, and admin-only actions in Insights, see [Role Permissions](/enterprise_onboarding/role_permissions).
...
```shell
docker run -it --rm -v ~/.specmatic:/root/.specmatic specmatic/enterprise get-license
```
...
```shell
docker run -it --rm -v "${env:USERPROFILE}/.specmatic:/root/.specmatic" specmatic/enterprise get-license
```
...
```shell
docker run -it --rm -v "%USERPROFILE%\.specmatic:/root/.specmatic" specmatic/enterprise get-license
```
...
:::info
User licenses acquired using the `get-license` command are short-lived (approximately 7 days). Specmatic auto-renews them in the background
while you continue using it. This ensures every user gets a short-lived license and licenses are not billed if a user is not using Specmatic
for a few days.
:::
...
```shell
docker run -it --rm -v ~/.specmatic:/root/.specmatic specmatic/enterprise show-license
```
...
```shell
docker run -it --rm -v "${env:USERPROFILE}/.specmatic:/root/.specmatic" specmatic/enterprise show-license
```
...
```shell
docker run -it --rm -v "%USERPROFILE%\.specmatic:/root/.specmatic" specmatic/enterprise show-license
```
...
For the difference between Organization Admin, Service Account, and Service Account Admin responsibilities, see [Role Permissions](/enterprise_onboarding/role_permissions).
...
[Insights Set Up Guide](/enterprise_onboarding/insights/setup#step-2-configuring-specmatic-insights) provides detailed instructions on how to set up and use the license key in CI/CD pipelines and automated systems.
