...
> Note: In V2 Config, adapters are configured only at the global level.
...
1. The **dependencies level request adapter** (`pre_specmatic_request_processor`) is executed initially
2. Specmatic tries to match the transformed request with a service
3. If a match is found and a service adapter is configured, the **service-level request adapter** is executed next
...
2. Following that, the **dependencies level response adapter** (`post_specmatic_response_processor`) is executed

---
### Important Behavior

If a request requires preprocessing before it can be matched to a specification, that logic **must be implemented at the global level**.
...
Incoming Request
  ↓
[Dependencies level pre_specmatic_request_processor]
  ↓
[Service level pre_specmatic_request_processor]
  ↓
[Execution]
  ↓
[Service level post_specmatic_response_processor]
  ↓
[Dependencies level post_specmatic_response_processor]
  ↓
Final Response
```
