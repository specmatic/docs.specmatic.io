...
| `servers[].server.client.producer` | Confluent serializer properties, including Registry client authentication and subject-name strategy. |
| `servers[].server.client.consumer` | Kafka consumer properties. Specmatic receives values as bytes, then decodes them using the referenced schema. |
