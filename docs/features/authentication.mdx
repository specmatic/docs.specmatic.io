...
## Injecting auth tokens using Template Values <CommercialBadge/>

If an auth token is already available as an environment variable or system property, you can use Template Values in the external example instead of hard-coding the token.

This is useful for contract tests where each environment provides its own token.
