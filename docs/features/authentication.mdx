...
Use example-level authentication when the credential belongs to a specific external example or test case.
This can be done by putting a value directly in the example, injecting a value into the example at runtime, or fetching a value just before the contract test runs.
...
<a id="injecting-auth-tokens-directly-into-examples-" />
...
<a id="just-in-time-authentication-" />
...
<a id="providing-fresh-auth-credentials-for-contract-tests" />
## Global Authentication

Use global authentication when you want Specmatic to apply credentials based on the OpenAPI security scheme, instead of putting credentials in each example.
...
<a id="important-security-scheme-names-must-match-the-openapi-spec" />
...
<a id="environment-variable-values-what-to-provide" />
...
<a id="recommended-configure-auth-in-specmaticyaml" />
...
<a id="testing-with-mock-auth-recommended-for-contractcomponent-tests" />
<a id="recommended-approach" />
## Mocking / Bypassing Authentication

For contract tests, you can isolate your application from real auth providers, such as OAuth servers, DB-backed auth, or identity services.
...
<a id="how-specmatic-chooses-authentication-for-a-request" />
<a id="precedence-rule" />
...
<a id="example-different-auth-schemes-per-operation" />
