...
## Auth using substitution <CommercialBadge/>

For contract tests, you can fetch a bearer token before the test runs and use it in the main request using substitution.
This is useful when your application expects an `Authorization` header, but you do not want to hard-code a token in every example or need a fresh token.

The OpenAPI spec may define bearer auth like this:
...
## Auth using Template Values <CommercialBadge/>

If the bearer token is already available as an environment variable or system property, you can use Template Values in the external example instead of hard-coding the token.
