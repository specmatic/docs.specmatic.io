...
When Specmatic generates requests while running tests or responses while running as a mock and no examples have been provided, Specmatic will generate values for fields based on the schema in the OpenAPI specifications.
...
## Automated Dictionary Generation <CommercialBadge/>
...
```shell
docker run --rm -v "$(pwd):/usr/src/app" specmatic/enterprise examples dictionary --spec-file employees.yaml
```
...
```shell
docker run --rm -v "$(pwd):/usr/src/app" specmatic/enterprise examples dictionary --spec-file employees.yaml --examples-dir ./examples
```
...
```shell
java -jar specmatic.jar mock employees.yaml
```
</TabItem>

<TabItem value="test-npm" label="NPM">
```shell
npx specmatic mock employees.yaml
```
</TabItem>

<TabItem value="test-docker" label="Docker">
```shell
docker run --rm --network host -v "$(pwd):/usr/src/app" specmatic/specmatic mock "employees.yaml"
```
</TabItem>
...
```shell
docker run --rm --network host -v "$(pwd):/usr/src/app" specmatic/specmatic test employees.yaml
```
</TabItem>
