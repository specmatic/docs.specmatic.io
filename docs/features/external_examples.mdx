...
Instead of hardcoding those values in the example, you can write a template expression with a default value. Specmatic resolves these expressions before using the example.
...
In this example, `API_TOKEN` is looked up at runtime, which can provided it in any of these ways:
...
With `API_TOKEN=secret-token`, the example resolves as follows:
```json
{
  "http-request": {
    "method": "GET",
