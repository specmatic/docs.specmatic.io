...
This won't work as an inline example. Even if you put an example for a 405 or 415 status response inline, the request needs to include an invalid method or Content-Type. OpenAPI does not have a place for these examples, and hence we full examples (including a request that triggers a 405 or 415 response) can't be put inline.
