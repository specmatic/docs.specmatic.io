...
When generating a JSON object from a schema that contains `anyOf`, one schema is selected at random and an object is generated from it based on that schema. This applies to example generation as well as response generation during mocking.
