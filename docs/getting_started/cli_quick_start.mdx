...
* Here's a detailed breakdown of the contract test:
  ...
  - **Response Validation:** Note that the specification runs by default as a contract test, and validates only the API signature, not the API logic. This is because responses often contain ephemeral values such as dates, timestamps, ids, etc. Thus, the test does not by default validate the actual response values defined in the `SCOOBY_200_OK` example against the values returned by the application. It only validates the response code. However, Specmatic does support making specific assertions about response values using the [`matchers` feature](/features/matchers).
...
```shell
npx specmatic mock service.yaml --port 9002
```
...
API Specification Summary: service.yaml
  ...
  >> Error loading stub expectation file '/private/tmp/service_examples/togo.json':
   /private/tmp/service_examples/togo.json didn't match service.yaml
