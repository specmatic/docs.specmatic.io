...
> **Download Specmatic:** You can download the latest Specmatic executable, Docker image, or Maven dependency from the [Download page](/download).

## Set up

Specmatic can be used as a standalone executable and also included [programmatically](/contract_driven_development/service_virtualization#programmatically-starting-stub-server-within-tests) as part of your test suite.
...
<Tabs groupId="install">
  ...
  <TabItem value="java" label="Java">
    ...
    - <a href={`https://github.com/specmatic/specmatic/releases/download/${VERSIONS['specmatic-core-version']}/specmatic.jar`}>GitHub</a>
    - <a href={`https://repo1.maven.org/maven2/io/specmatic/specmatic-executable-all/${VERSIONS['specmatic-core-version']}/specmatic-executable-all-${VERSIONS['specmatic-core-version']}.jar`}>Maven Central</a>

    To run the JAR:

    <CodeBlock language="bash">java -jar specmatic.jar --help</CodeBlock>

    For convenience methods (aliases and install scripts), see the [Download page](/download).
...
<YamlDisplay object={yaml.load(require('./includes/cli-quick-start/service.yaml?raw'))}/>

---

## Create a Specmatic configuration (v3)

Create a file named `specmatic.yaml` next to `service.yaml`. This is the v3 configuration format defined in `specmatic-schema.json` (repo root).

<V3SpecmaticJsonYamlTabs
  v3Object={yaml.load(require('./includes/cli-quick-start/specmatic.yaml?raw'))}/>
...
```shell
docker run -v "$(pwd):/usr/src/app" specmatic/specmatic test
```

**Windows Command Prompt**

```shell
docker run -v "%cd%:/usr/src/app" specmatic/specmatic test
```
...
Using Specmatic Enterprise license initialized from /Users/ketan/.specmatic/license.json

--------------------
  Request to https://my-json-server.typicode.com/specmatic/specmatic-documentation-examples at 2026-2-17 7:5:51.253
  ...
  Response at 2026-2-17 7:5:52.170
    200 OK
    Date: Tue, 17 Feb 2026 13:35:52 GMT
...
Generating report for 1 tests...
Generating CTRF report in build/reports/specmatic/test/ctrf/ctrf-report.json
Generating HTML report in build/reports/specmatic/test/html/index.html


Tests run: 1, Successes: 1, Failures: 0, Errors: 0

Executed at 2026-02-17T19:05:52.489073
```
...
Unsuccessful Scenarios:
  " Scenario: GET /pets/(petid:number) -> 200 with the request from the example 'SCOOBY_200_OK' FAILED"
        Reason:
    Testing scenario "Should be able to get a pet by petId. Response: Should be able to get a pet by petId"
...
```
--------------------
  Request to https://my-json-server.typicode.com/specmatic/specmatic-documentation-examples at 2026-2-17 7:7:35.685
    GET /specmatic/specmatic-documentation-examples/pets/227.0
    ...


  Response at 2026-2-17 7:7:36.616
    404 Not Found
    Date: Tue, 17 Feb 2026 13:37:36 GMT
    Content-Type: application/json; charset=utf-8
    ...
```
...
Unsuccessful Scenarios:
  "Load Error FAILED"
        Reason:
    Error loading example named SCOOBY_200_OK for GET /pets/(petid:number) -> 200
      ...
      >> RESPONSE.BODY.status
          ...
          Specification expected type string but example "SCOOBY_200_OK" contained value true of type boolean
...
```
    UI (Consumer)         Specmatic Mock <- service.yaml
          | --- getPetById ---> |
          | <-- {Pet JSON} ---- |
```
...
To spin up a mock server with the `service.yaml` we authored earlier, update `specmatic.yaml` to include the `mock` command (shown below). The default port is `9000`.

**Tip:** You can switch the port number by updating the port field in the `runOptions` configuration in `specmatic.yaml` file as shown below.

<V3SpecmaticJsonYamlTabs
  v3Object={yaml.load(require('./includes/cli-quick-start/specmatic-mock.yaml?raw'))}/>
...
```shell
docker run -v "$(pwd):/usr/src/app" --network host specmatic/specmatic mock
```

**Windows Command Prompt**

```shell
docker run -v "%cd%:/usr/src/app" --network host specmatic/specmatic mock
```
...
```shell
curl http://localhost:9002/pets/123
```
...
[Example SCOOBY_200_OK]: Error from contract service.yaml
  ...
  API: GET /pets/(petid:number) -> 200
    ...
    >> RESPONSE.BODY.status
        ...
        Specification expected mandatory property "status" to be present but was missing from the example "SCOOBY_200_OK"
...
```shell
npx specmatic mock service.yaml
```
...
API Specification Summary: service.yaml
  ...
  1 examples(s) found in ./service_examples
...
API Specification Summary: ./service.yaml
  ...
  >> Error loading stub expectation file './service_examples/togo.json':
   ./service_examples/togo.json didn't match service.yaml
    Error from contract service.yaml
      ...
      API: GET /pets/(petid:number) -> 200
        ...
        >> RESPONSE.BODY.status
            ...
            Specification expected mandatory property "status" to be present but was missing from the example
