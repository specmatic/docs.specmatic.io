...
sidebar_label: Studio Quick Start
sidebar_position: 1
---
...
Click on `Spec` tab, restore `petstore.yaml` back to its original state by pasting [this content](/original_petstore_spec) back in, save it, and run the tests. With the example back, the tests will pass once again.
...
Please restore `petstore.yaml` to its [original state](/original_petstore_spec) before proceeding with this section.
