...
<Tabs groupId={`tab-complete-examples`}>
  <TabItem value="v3" label="Version 3 (recommended)">
    ...
    **Configuration Structure Summary (v3)**
    ...
    3. **systemUnderTest** - Defines the primary system under test. Usually references a `components.services` entry and
    an associated `runOptions` profile for executing contract tests.
