...
The Specmatic configuration file provides a centralized way to define how Specmatic should operate within your project. It allows you to declaratively specify the specifications to load, the test mode to run in, the mock settings, and other runtime options required by Specmatic.

Using this configuration file helps keep all Specmatic-related settings together so you don’t need to update multiple places when making changes. It simplifies setup, reduces repetition, and makes your Specmatic usage predictable and easy to understand—whether you are working with consumer tests, provider tests, or using Specmatic as a mock.
...
* `service.runOptions` defines *how* that service is run (test vs mock, base URL, filters)

Optional `service.data.examples` directories provide example payloads used during testing.
`systemUnderTest` itself accepts only `service`; keep service-specific `runOptions`,
`data`, and `settings` inside that object.
...
* Each dependency entry contains a `service`; keep its service-specific
  `runOptions`, `data`, and `settings` inside that object
* Multiple dependencies can be declared
* Dependency-wide `data` and `settings` may instead be defined directly under
  `dependencies` when they genuinely apply to every dependency service
...
* `type: mock` starts a Specmatic mock server
...
By consolidating these details into a dedicated configuration file, Specmatic allows your project to remain clean and organized while giving you full control over how contract tests and mocks behave.
