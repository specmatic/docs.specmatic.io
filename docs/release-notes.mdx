...
- Improved isolation of Arazzo runtime inputs, step outputs, and workflow outputs under their workflow namespace, while preserving local shorthand and explicit cross-workflow runtime expressions.
- Improved drag-and-drop Arazzo workflow editing in Studio, including proximity connections and auto workflow resizing
- Improved Arazzo workflow editor layout so workflow containers resize around dropped steps.
- Improved Arazzo support for multiple workflow inputs, shared defaults, and input isolation.
- Improved source resolution for single-source specifications and reusable references.
- Improved handling of optional bodies, default response statuses and scalar subtypes.
- Improved Arazzo error messages and validation diagnostics
...
- Added official Arazzo 1.0.x and 1.1.x schema handling, including generation using the official 1.1.x schema.
- Added Safari-compatible workflow-editor drag-and-drop, full-size server-picker rendering, and reliable workflow rendering.
