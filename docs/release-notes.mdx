...
- Updated the bundled Specmatic runtime to `2.47.0`.
- Updated `org.apache.activemq:artemis-jakarta-client` and `artemis-jms-client` to `2.54.0` to address security vulnerabilities.
- Updated `org.apache.activemq:activemq-kahadb-store` and `activemq-broker` to `v6.2.6`.
- Updated `com.rabbitmq:amqp-client` to `v5.31.0`.
- Updated `com.graphql-java:graphql-java` to `v26`.
- Updated the `com.google.protobuf` plugin to `v0.10.0`.
- Switched to `grpc-netty` to ensure compatibility with Netty `4.2`.
