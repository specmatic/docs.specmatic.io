...
Start a mock server for async messaging protocols. Supports both external brokers (configured in `specmatic.yaml`) and in-memory Kafka broker.

```bash
specmatic-enterprise mock [OPTIONS]
```
