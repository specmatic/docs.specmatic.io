...
- [IBM MQ](/supported_protocols/asyncapi/ibm-mq)
- [Google Pub/Sub](/supported_protocols/asyncapi/google-pub-sub)

For runtime broker configuration, server references, and mock/test options, see [AsyncAPI Configuration](/supported_protocols/asyncapi/configuration).
...
For Kafka contracts backed by Confluent Schema Registry, see [Confluent Schema Registry](/features/asyncapi/confluent-schema-registry).
...
The command uses each server's explicit `type` to connect to an external broker or start a supported in-memory broker. See [AsyncAPI Configuration](/supported_protocols/asyncapi/configuration) for precedence and migration details.
...
The [AsyncAPI Configuration](/supported_protocols/asyncapi/configuration) guide explains external and in-memory servers, inline and referenced servers, shared defaults, per-spec overrides, credentials, Schema Registry, and migration from `inMemoryBroker`.
