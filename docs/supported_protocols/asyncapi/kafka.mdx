...
**CLI Arguments:**
- `--host` - Kafka broker host and port (e.g., localhost:9092)
...
This command requires the `--host` CLI argument to specify the Kafka broker connection.
...
contracts:
  - git:
    ...
    provides:
      - specs:
          - /path/to/your/order_service_async.yaml
        specType: asyncapi
        config:
          servers:
            - host: localhost:9092
...
:::info
When using the simple version 2 configuration, you need to provide the Kafka broker configuration through CLI arguments when running tests.
:::
...
Replace `kafka_broker_host` and `kafka_broker_port` with appropriate values. This command requires CLI arguments since the Kafka broker configuration is not in the YAML file.
...
:::info
When using the simple version 2 configuration, you need to provide Kafka server configuration through CLI arguments when running tests.
:::
