...
If you want Specmatic to use an existing Kafka broker instead of the embedded one, update the server in `specmatic.yaml`:
...
Then run the same `specmatic/enterprise mock` command. Specmatic connects to the configured broker and does not manage its lifecycle.
...
- `servers[].server.host` and `servers[].server.protocol` select the broker
- `servers[].server.type` chooses an external or Specmatic-managed broker
- `servers[].server.adminCredentials` carries Kafka admin client properties
- `servers[].server.client.producer` and `servers[].server.client.consumer` carry Kafka client properties
