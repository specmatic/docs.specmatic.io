...
**5.** Run the following command to start the GraphQL stub on `localhost:9000/graphql`:
   ```bash
   docker run --network host -v "$(pwd):/usr/src/app" specmatic/enterprise mock --port=9000 --examples=examples
   ```
...
4. **Run the GraphQL Stub**:
   ```bash
   docker run -v "$(pwd):/usr/src/app" specmatic/enterprise mock --port=9000 --examples=examples
   ```
