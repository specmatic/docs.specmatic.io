...
```bash
docker run --network host -v "$(pwd):/usr/src/app" specmatic/enterprise mock --examples=examples
```
...
contracts:
  - git:
    ...
    provides:
      - specs:
          - /path/to/your/products_bff.graphqls
        specType: graphqlsdl
        config:
          host: localhost
          port: 8080
...
This command requires the `SPECMATIC_GENERATIVE_TESTS` environment variable and additional CLI arguments (`--port` and `--host`) to specify where the GraphQL service under test is running, since this configuration is not in the YAML file.
...
:::info
When using the simple version 2 configuration, you need to provide additional configuration through CLI arguments:

**CLI Arguments:**
- `--host` - GraphQL service host
- `--port` - GraphQL service port
