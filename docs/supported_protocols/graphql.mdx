...
GraphQL
  ...
  - Mocking out specific values using example data
...
* instructions on how to start mock server using graphql spec with custom examples, simulating delays, errors etc.
...
To start a mock server using Specmatic, you can use the following steps:
...
Run the following command to start the GraphQL mock:

```bash
docker run --network host -v "$(pwd):/sandbox" specmatic/enterprise mock /sandbox/product-api.graphql
```

That's it! You now have a GraphQL mock server running on `localhost:9000/graphql` serving the schema defined in `product-api.graphql`.
...
```bash
docker run --network host -v "$(pwd):/spec" specmatic/enterprise mock /spec/product-api.graphql
```
...
By convention, Specmatic automatically loads all examples defined for the GraphQL SDL file, say `<graphql_sdl_filename>.graphql`, from a colocated directory called `<graphql_sdl_filename>_examples`. However, in case your examples are in a different directory, you can configure it as follows:
...
components:
  ...
  services:
    yourService:
      description: Your GraphQL Service
      definitions:
        - definition:
            source:
            ...
            specs:
              - spec:
                  id: yourServiceSpec
                  path: /path/to/your/service.graphqls
  runOptions:
    yourServiceMock:
      graphqlsdl:
        type: mock
...
Set the `EXAMPLES_DIR` system property with a comma-separated list of directory paths:

```java
System.setProperty("EXAMPLES_DIR", "./custom-examples,./more-examples");
```

**CLI Approach:**

Specify example directories using the `--examples` argument:

```bash
--examples=./custom-examples --examples=./more-examples
```
...
When running a GraphQL mock using Specmatic, you can provide an example query that includes all possible fields and sub-selections. Specmatic uses this example to generate the mock response.
...
dependencies:
  services:
    - service:
        ...
        runOptions:
          $ref: "#/components/runOptions/productApiServiceMock"
        data:
          examples:
            - directories:
                - examples
...
**5.** Run the following command to start the GraphQL mock on `localhost:9000/graphql`:
...
The Specmatic GraphQL mock server supports multi-field queries, allowing you to send a single request with multiple queries and receive a consolidated response. This feature is useful when you want to retrieve data from different queries in a single API call. Additionally, **multi-field queries with variables** are supported, making it flexible for dynamic requests.
...
This request showcases how Specmatic's GraphQL mock server can process multi-query requests with variables, offering flexibility and efficiency in response generation.
...
To run contract tests, you need to configure your `specmatic.yaml` file to include the system under test:

```yaml
version: 3
systemUnderTest:
  service:
    $ref: "#/components/services/productsBffService"
    runOptions:
      $ref: "#/components/runOptions/productsBffServiceTest"
dependencies:
  services:
    - service:
...
components:
  ...
  services:
    productsBffService:
      description: Products BFF GraphQL Service
      definitions:
        - definition:
            source:
              $ref: "#/components/sources/yourContracts"
            specs:
              - spec:
                  id: productsBffSpec
                  path: /path/to/your/products_bff.graphqls
    orderApiService:
      ...
      definitions:
        - definition:
            ...
            specs:
              - spec:
                  id: orderApiSpec
                  path: /path/to/your/api_order.yaml
  runOptions:
    productsBffServiceTest:
      graphqlsdl:
        type: test
        specs:
          - spec:
...
This command mounts your `specmatic.yaml` file and runs tests against your service. All configuration (host, port, generative tests, etc.) is read from the YAML file.

This command mounts your `specmatic.yaml` file and runs tests against your service by generating GraphQL requests based on the GraphQL SDL files listed under `provides` or `systemUnderTest` section.
...
Make sure to update the repository URLs, paths, and configuration to reflect your actual contract repository and .graphqls file locations.
...
Specmatic ships with the GraphiQL GUI, an in-browser GraphQL IDE, that you can use to explore and test your GraphQL APIs. You can access it at `http://localhost:9000/graphiql` when your Specmatic GraphQL mock server is running.
