    ...
    provides:
      - specs:
        ...
        config:
          host: localhost
          port: 9090
          importPaths: # List of paths to resolve imports from, for example if your proto file imports other proto files from a different directory then you can specify the path to that directory here
            - ../
          protocVersion: 3.23.4
          requestTimeout: 5000 # Duration in milliseconds to wait for a response before timing out. Default is 5000 ms (5 seconds).
```
