...
This operation was skipped because it did not match the [**filter**](./contract_driven_development/contract_testing#running-specific-tests)

**Why this is a problem**

This is an issue if you anticipated the operation to execute, but it did not.

**How this can be resolved**

Verify that the operation meets the filter criteria.
...
- This is an issue if you anticipated the operation to execute, but it did not.
- By default, only operations with a `2xx` status are generated automatically.
- Operations that do not return a `2xx` status require explicit examples to be provided.
- Except `400` operations, which can be auto-generated if [**Schema Resiliency**](./references/configuration/test-configuration#resiliency-tests) is enabled.

**How this can be resolved**

Please provide examples for the operation.

## T00003

### Examples Required in Strict Mode

[Strict mode](./contract_driven_development/contract_testing/#strict-mode) requires at least one example, but none were found for this operation

**Why this is a problem**
- This is an issue if you anticipated the operation to execute, but it did not.
- When strict mode is **enabled**, only operations with valid examples will be executed.
...
This operation was skipped because it required generation to be enabled

**Why this is a problem**
- This only happens for the bad request responses.
- This is an issue if you anticipated the operation to *execute,* but it did not.
...
**Why this is a problem**
- This is expected only in [Studio SaaS](https://studio.specmatic.io).
- Studio SaaS limits how many tests can run in a single test run.

**How this can be resolved**

If this issue arises outside of Studio SaaS, please contact the Specmatic team for assistance.

## T00006

### Accept Mismatch

The request [Accept header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Accept) does not match the response content type of the operation
...
This operation was executed using an available example, the example could've been inline or external

**What this means**

This is an issue if you didn't anticipate the operation to be *executed,* but it was.

**How to control this behavior**

Please remove or delete the relevant example.

## T10002

### Executed Using Generation

This operation was executed by generating payloads, due to the absence of an example

**What this means**

This is an issue if you *didn't* anticipate the operation to be executed using generation.

**How to control this behavior**
- Provide an example for the operation.
- If an example already exists, confirm that it is valid and has been loaded by Specmatic.

## T10003

### Executed Using Positive Generation

This operation was executed by generating +ve payloads, due to positive generation being enabled

**What this means**

- This is an issue if you *didn't* anticipate the operation to be *executed* using positive generation.
- This issue can arise even when a valid example is provided for the operation.

**How to control this behavior**

To resolve this, disable positive generation in the [Specmatic Configuration](/references/configuration/test-configuration#resiliency-tests).

## T10004

### Executed Using Negative Generation

This operation was executed by generating -ve payloads, due to negative generation being enabled

**What this means**
- This is an issue if you *didn't* anticipate the operation to be *executed* using negative generation.
- This issue can arise even when a valid example is provided for the operation.

**How to control this behavior**

Disable negative generation in the [Specmatic Configuration](/references/configuration/test-configuration#resiliency-tests).
