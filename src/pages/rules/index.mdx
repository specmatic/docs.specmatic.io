...
This operation was skipped because it did not satisfy the specified [**filter**](./contract_driven_development/contract_testing#running-specific-tests)
...
Verify that the operation meets the specified filter criteria.
...
**Why this is a problem**
- This is an issue if you anticipated the operation to *execute,* but it did not.
- When examples are absent, Specmatic can generate test inputs from the contract schema. Resiliency testing is also what enables broader positive and negative combinations beyond explicit examples. 

**How this can be resolved**
Enable generation in your [Specmatic Configuration](./references/configuration/test-configuration#resiliency-tests).
...
**Why this is a problem**
- This issue should be restricted to [Studio SaaS](https://studio.specmatic.io) and should not arise in other contexts.
- The SaaS imposes a limit on the number of tests that can be executed in a single test run.
...
This operation was executed using an an available example, the example could've been inline or external
